Shader "Test /Floor" {
	...
	_ReflectionAlpha("ReflectionAlpha", Range(0,1)) = 0.5
		[HideInInspector]_ReflectionTex("", 2D) = "white" {}
	[Enum(Off, 0, On, 2)] _Cull("Cull Mode", Float) = 2
		_Color("Color", Color) = (0.5,0.5,0.5,1)
		__FloorTex("MainTex", 2D) = "white" {}
...
#pragma multi_compile_fullshadows_refl 
#pragma target 3.0
#pragma glsl	
		...
		uniform sampler2D _FloorTex; uniform half4 _FloorTex_ST;
		...
		half4 _FloorTex_var = tex2D(_FloorTex,TRANSFORM_TEX(i.uv0, _FloorTex));
		...
		half3 node_1778 = (node_3600*_Color.rgb*_FloorTex_var.rgb*
			(1.0 - ((node_9165.r + node_9165.g + node_9165.b)*0.3333333)));
		...
		half3 node_2276 = DynamicStatic(ProbeLighting(i.normalDir , node_11 , node_2036)
			, node_2036 , node_11); // GI
		half node_56 = max(0,dot(normalDirection,viewDirection)); // NdotV
		half3 emissive = ((UNITY_LIGHTMODEL_AMBIENT.rgb*node_1778) +
			(node_2276*node_1778*_Doubleincominglight_var));
		...
		half3 node_610 = normalize(cross(normalDirection,mul(half3(1,0,0),
			tangentTransform).xyz.rgb)); // tangent
		...
		half node_3246 = V(node_11, (node_2558 + node_2567) *
			_RoughnessPower * _RoughnessPower, node_56);
		half node_1138 = max(0,dot(node_101,lightDirection)); // HdotL
		half node_111 = (1.0 - node_1138);
		half3 node_116 = (node_2868 +
			((node_111*node_111*node_111*node_111*node_111)*(1.0 - node_2868))); // Fresnel Term
		half3 finalColor = emissive + lerp(node_1289,
			(((node_11*(exp((-1 * (((node_643*node_643) + (node_644*node_644)) / (node_96*node_96))))
				*normTerm(node_3246))*(1.0 / node_3246)*node_116*3.141592654)
				*node_2036*_RoughnessRG_var.b*_SpecularColor.a) + node_1289)
			, _UseSpecular);

		fixed3 refCol = tex2D(_ReflectionTex, i.refl.xy / i.refl.w).xyz;

		return fixed4((finalColor * (1 - _ReflectionAlpha) +
			refCol * (_ReflectionAlpha)), 1);
		}
			ENDCG
		}
	}
