...
public class WizardBakeVertAO : ScriptableWizard
{
	...
	void OnWizardCreate ()
	{
        ...
        //GameObject debugSphere = GameObject.FindWithTag("Debug");
		...
		foreach (MeshFilter mf in mfs) {
			...
			for (i=0; i<l; i++) {
				...
				for (j=0; j<samples; j++) {
                    ...
                    Physics.IgnoreLayerCollision(LayerMask.NameToLayer("Wall"), LayerMask.NameToLayer("Default"));
                    if ( Physics.Linecast( v-(offset*0.1f), v + ray, out hit ) && (hit.collider.tag == "Wall" || hit.collider.tag == "Floor")) {
                        
                        //Debug.Log(Physics.GetIgnoreLayerCollision(LayerMask.NameToLayer("Wall"), LayerMask.NameToLayer("Wall")));
                        /*
